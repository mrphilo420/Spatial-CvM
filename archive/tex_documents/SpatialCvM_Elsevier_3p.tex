...
\begin{equation}
T_n \stackrel{d}{\longrightarrow} \sum_{m=1}^{K-1} \lambda_m^* \cdot \chi^2_{1,m}
\end{equation}
where $\lambda_m^*$ are the first $K-1$ (nonzero) eigenvalues of the contrast covariance operator, and $\chi^2_{1,m}$ denotes independent chi-square variables with 1 degree of freedom each.
\end{theorem}
...
\textbf{Step 3: Spectral Representation.} Substituting into the Mercer decomposition:
\begin{equation}
\int_0^1 \mathcal{G}(y)^2 \, dH_0(y) = \sum_{m=1}^{K-1} \lambda_m^* Z_m^2 = \sum_{m=1}^{K-1} \lambda_m^* \chi^2_{1,m}
\end{equation}
The contrast subspace has dimension $K-1$ (one constraint $\sum_k h_k = 0$ in $K$ dimensions), so there are exactly $K-1$ nonzero eigenvalues.
\end{proof}
...
\begin{equation}
T_n^{(p)} \stackrel{d}{\longrightarrow} \sum_{m=1}^{K-1} \lambda_m^{*,(p)} \cdot \chi^2_{1,m}
\end{equation}
where $\lambda_m^{*,(p)}$ are the first $K-1$ (nonzero) eigenvalues of the multivariate contrast covariance operator.
\end{theorem}
...
Induction on $n$ using telescoping sum properties. Location: \texttt{Theorem2/DiscreteCvM.lean} (imported via main library), lemma \texttt{abel\_sum\_identity}
\end{proof}
...
Location: \texttt{Proofs/LagRegroupProof.lean} (imported via main library). The proof uses \texttt{Finset.sum\_bij} for bijective reindexing, establishing the combinatorial identity. Full de-axiomatization status: \textbf{est.~4--8 weeks}
\end{proof}

\begin{theorem}[Davydov's Inequality (AXIOMATIZED in Lean)]
    \label{thm:davydov-full}
    For α-mixing processes with coefficient α(d), and functions f ∈ L^∞(𝒜), g ∈ L^∞(ℬ):
    \begin{equation}
        |\text{Cov}(f, g)| \leq 12 \cdot \alpha(\mathcal{A},\mathcal{B}) \cdot \|f\|_\infty \cdot \|g\|_\infty
    \end{equation}
\end{theorem}

\begin{theorem}[Davydov-Rio Inequality with L^p Optimization (AXIOMATIZED in Lean)]
    \label{thm:davydov-rio}
    For centered indicator functions with Rio exponent δ/(2+δ) (Rio 2013, Theorem~1.1):
    \begin{equation}
        |\text{Cov}(\mathbb{1}\{X_0 \leq x\} - F(x), \mathbb{1}\{X_d \leq y\} - F(y))| \leq 4 \cdot \alpha(d)^{\frac{\delta}{2+\delta}}
    \end{equation}
    where δ \textgreater{} 0 is arbitrary. The exponent δ/(2+δ) ensures summability when α(d) decays geometrically.
\end{theorem}

\begin{proof}[De-axiomatization Path]
Location: \texttt{Lemma1/Mixing.lean}. The \textbf{critical path} involves:
\begin{enumerate}
    \item \texttt{sigma_algebra_inclusion}, \texttt{indicator_measurable}: $\sigma$-algebra inclusion lemmas for Davydov application (from Math StackExchange insight)
    \item \texttt{davydov_rio_inequality}: Full Rio/Davydov inequality with exponent $\frac{\delta}{2+\delta}$ (est.\ 6--12 months for Mathlib $L^p$ infrastructure)
...
\begin{itemize}
    \item \textbf{PROVED:} Abel summation, Lag regroup framework, basic kernel properties
    \item \textbf{AXIOMATIZED:} Davydov-Rio inequality with exponent $\frac{\delta}{2+\delta}$ (Rio 2013), Prokhorov's theorem, Mercer decomposition
