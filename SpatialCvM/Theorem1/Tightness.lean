...
-- Axiomatize: Hölder continuity implies uniform continuity
-- This is a standard result in analysis (follows from uniform continuity on compact sets)
axiom holder_continuity (f : ℝ → ℝ) (γ : ℝ) (hγ : 0 < γ ∧ γ ≤ 1)
    (h : ∀ x y, |f x - f y| ≤ |x - y| ^ γ) :
    UniformContinuousOn f (Set.Icc 0 1)

-- ============================================================================
-- HELPER AXIOM: Arzelà-Ascoli (Compactness from Boundedness + Equicontinuity)
-- Axiomatize: This is a deep theorem in empirical process theory
-- States: Boundedness + equicontinuity ⟹ tightness (in weak topology)
...
lemma empirical_process_bounded (K : ℝ → ℝ) (h : ℝ) (hh : h > 0)
  ...
  have h_sup_le_B : sup_norm K (Set.Icc (-1) 1) ≤ B := by
    exact sup_norm_le_of_bound (fun x _ => hB_bound x)
